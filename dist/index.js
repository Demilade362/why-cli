...
program
    ...
    try {
        ...
        let byFile = new Map();
        let totalRefs = 0;
        let importance = computeImpact(0, 0);
        let transitiveMap = null;
        if (opts.transitive) {
        ...
        if (opts.json) {
            ...
            if (transitiveMap)
                out.transitive = Array.from(transitiveMap.entries()).map(([depth, set]) => ({ depth, files: Array.from(set) }));
        ...
        console.log(pc.bold(pc.cyan(`  🔎  WHY — ${rel}`)), pc.dim(`  (${variants.map((v) => v).join(", ")})`));
        ...
        if (git) {
            ...
            if (author)
                console.log("  " + pc.bold("Introduced by:") + " " + pc.cyan(author));
        }
        else {
