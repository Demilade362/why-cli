...
import { getRepoIgnore, HardIgnore } from "./scan.js";
import type { FoundRef, ScanOptions } from "../types.js";

type SpecHit = { spec: string; line: number; text: string };

function extractSpecifiers(text: string): SpecHit[] {
...
function tryResolveRelative(importerDir: string, spec: string): string | null {
...
export function buildImportGraph(cwd: string, opts: ScanOptions = {}) {
  ...
  const specHitsMap = new Map<string, SpecHit[]>();
  ...
  for (const file of files) {
    ...
    for (const h of hits) {
      ...
      if (spec.startsWith(".")) {
        ...
        if (resolved) {
          const set = importsMap.get(relFile) ?? new Set<string>();
          set.add(resolved);
...
export function findDirectImportersForTarget(cwd: string, targetRel: string, graph: ReturnType<typeof buildImportGraph>) {
  const { importsMap, specHitsMap } = graph;
  const byFile: Map<string, FoundRef[]> = new Map();
...
export function findTransitiveImporters(target: string, graph: ReturnType<typeof buildImportGraph>, maxDepth = 10) {
  const { importsMap } = graph;
  // Build reverse map
  const reverse = new Map<string, Set<string>>();
  for (const [file, deps] of importsMap.entries()) {
    for (const d of deps) {
      const set = reverse.get(d) ?? new Set<string>();
      set.add(file);
      reverse.set(d, set);
    }
  }

  const q: Array<{ file: string; depth: number }> = [];
  ...
  while (q.length > 0) {
    ...
    const parents = reverse.get(cur.file) ?? new Set<string>();
    for (const p of parents) {
      if (seen.has(p)) continue;
      const d = cur.depth + 1;
      const set = result.get(d) ?? new Set<string>();
      set.add(p);
      result.set(d, set);
      seen.add(p);
